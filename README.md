...
このシステムはclient-serverモデルに従い作成されました。フロントエンドは`React`，バックエンドは`Node.js`を使用して構築しました。データベースはnoSQLの`Mongodb`を導入しました。フロントエンドはガチャのRequestをAjaxの形式でバックエンドのNodejsサーバーに送り、そしてnodeサーバはDBをサーチして必要な情報を獲得し、それをResponseとしてフロントエンドに返します。最後に、フロントエンドがその情報を処理して、UIを通じてユーザに示します。
...
#### 使用
このシステム作動環境はnodejsです。そしてmongodb，express，corsなどのpkgも必要です。もしあなたのコンピュータの中にnodejsの環境とpkgが揃えて、それに加えてGitBushが使用可能であれば，直接に
